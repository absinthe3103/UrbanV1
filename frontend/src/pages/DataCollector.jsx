...
const DataCollector = () => {
  ...
  const calculateFS = (data) => {
    ...
    if (!cohesion || !normalStress || !frictionAngle || !shearStress)
      return null;
    const phiRad = (parseFloat(frictionAngle) * Math.PI) / 180;
    const fs =
      (parseFloat(cohesion) + parseFloat(normalStress) * Math.tan(phiRad)) /
    ...
    <div style={styles.container}>
      <header style={styles.header}>
        ...
        <p style={styles.subtitle}>
          Specify building type and parameters for risk assessment
        </p>
      </header>
      ...
      <div style={styles.inputCard}>
        ...
        >
          ...
          <select
            ...
            onChange={(e) =>
              setCurrent({ ...current, buildingType: e.target.value })
            }
          >
            ...
            {rows.map((r) => (
              <tr key={r.id} style={styles.tr}>
                ...
                <td style={styles.td}>
                  <span
                    style={
                      r.fs < 1.2 ? styles.badgeDanger : styles.badgeSuccess
                    }
                  >
      ...
      <button
        onClick={async () => {
          try {
            ...
            alert(
              `Backend not found. Make sure your Python server is running on port 8000.\n${err.message}`,
            );
          }
        }}
